...
function Form() {
  ...
  const save=()=>{
    ...
    sessionStorage.setItem('number', formValues.number)
    sessionStorage.setItem('password', formValues.password)
    sessionStorage.setItem('repeatpassword', formValues.repeatPass)
  }
  // <>
...
const repeatpassword = sessionStorage.getItem('repeatpassword')
  return (
    <div className="container">
      {/* if keys in the object error is 0 and all the fields are filled then conditon */}
      
  
        {name && email && number && password && repeatpassword && password === repeatpassword && number.length === 10?(
           <>
                <h1 className="message">Thank You! You have successfully Signed in </h1>
                 
                 <Link to="/">
                 <button className="button" >Let's Go Back</button>
                   </Link>
            </>
        ):(
